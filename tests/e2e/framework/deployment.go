...
func (i *Invocation) GetDeployment(name string) *appsv1.Deployment {
	image := "nginx:latest"
	replicas := int32(2)
...
func (f *Framework) EventuallyImageClonedForDeploymentToBackupRegistry(meta metav1.ObjectMeta, registry string) GomegaAsyncAssertion {
	return Eventually(
		func() bool {
